...
class LoggingTimer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            self.logger.debug(f"{self.operation_name} tamamlandı, Süre: {duration:.2f} saniye")logger.error(f"{self.operation_name} hata ile sonlandı: {exc_val}, Süre: {duration:.2f} saniye")
        else:
            self.